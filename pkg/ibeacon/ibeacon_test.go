...
func TestBLEEvent(t *testing.T) {
	...
	for _, tc := range tt {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			bleEvent, err := got.EncodeBLEEvent()

			if err != nil {
				t.Errorf("did not expect error here. Yet. %s", err.Error())
			}

			if len(bleEvent) != 25 {
				t.Errorf("Expected length of 25, got %d", len(bleEvent))
			}
			if !reflect.DeepEqual(bleEvent, tc.data) {
				t.Errorf("Exepected (%d) %x, got (%d) %x", len(tc.data), tc.data, len(bleEvent), bleEvent)
			}
		})
	}
}
